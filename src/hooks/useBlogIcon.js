  ...
  useEffect(() => {
    ...
    function onUp() {
      ...
      if (isIconDroppedOnTarget(ref, binRef)) {
        setVisible(false)
        addItemToBin && addItemToBin({ id: 'blog', name, icon: blogIcon })
        return
      }
      if (folderRef && isIconDroppedOnTarget(ref, folderRef)) {
        setVisible(false)
        onDroppedIntoFolder && onDroppedIntoFolder({ id: 'blog', name, icon: blogIcon })
        return
      }
      if (getExtraFolderTargets) {
        ...
        for (const t of targets) {
          ...
          if (!(selfRect.right < r.left || selfRect.left > r.right || selfRect.bottom < r.top || selfRect.top > r.bottom)) {
            setVisible(false)
            onDroppedIntoExtraFolder && onDroppedIntoExtraFolder({ id: 'blog', name, icon: blogIcon }, t.id)
            return
          }
        }
      }
