...
export function useStoryIcon(binRef, addItemToBin, folderRef, onDroppedIntoFolder, getExtraFolderTargets, onDroppedIntoExtraFolder) {
  const [pos, setPos] = useState({ x: 110, y: 120 }) 
  ...
  const [name, setName] = useState('Our Story')
  ...
  useEffect(() => {
    ...
    function onUp() {
      ...
      if (isIconDroppedOnTarget(ref, binRef)) {
        setVisible(false)
        addItemToBin && addItemToBin({ id: 'story', name, icon: storyIcon })
        return
      }
      if (folderRef && isIconDroppedOnTarget(ref, folderRef)) {
        setVisible(false)
        onDroppedIntoFolder && onDroppedIntoFolder({ id: 'story', name, icon: storyIcon })
        return
      }
      if (getExtraFolderTargets) {
        ...
        for (const t of targets) {
          ...
          if (!(selfRect.right < r.left || selfRect.left > r.right || selfRect.bottom < r.top || selfRect.top > r.bottom)) {
            setVisible(false)
            onDroppedIntoExtraFolder && onDroppedIntoExtraFolder({ id: 'story', name, icon: storyIcon }, t.id)
            return
          }
        }
      }
  ...
  : { left: 110, top: 120, position: 'fixed', zIndex: dragging ? dragZ : baseZ }
  ...
  return {
    ...
    context,
    closeContext,
    deleteSelf: () => { if (!visible) return; setVisible(false); closeContext(); addItemToBin && addItemToBin({ id: 'story', name, icon: storyIcon }) },
    restore: () => {
      setVisible(true)
    ...
    copyDescriptor: () => ({ id: 'story', name, icon: storyIcon })
  }
}
